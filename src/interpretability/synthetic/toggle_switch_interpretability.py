...
from dataset.synthetic.toggle_switch_dataset import ToggleSwitchOracleDataset
from interpretability.oracle_interpretability import OracleMILInterpretabilityStudy


class ToggleSwitchOracleInterpretabilityStudy(OracleMILInterpretabilityStudy):

    dataset_clz = ToggleSwitchOracleDataset
    ...
    def plot_reward_overall(self, axis, plotting_data):
        reward_preds = plotting_data['reward_preds']
        reward_targets = plotting_data['reward_targets']
        axis.scatter(reward_preds, reward_targets, alpha=0.5, marker='x')
    ...
    def plot_reward_vs_trigger(self, axis, plotting_data):
        reward_preds = plotting_data['reward_preds']
        switch_state = plotting_data['switch_state']
        axis.scatter(switch_state, reward_preds, marker='.', alpha=0.5)
        axis.set_xlim(-0.05, 1.05)
        axis.set_ylim(self.min_reward, self.max_reward)
        axis.set_xticks([0, 1])
        axis.set_xticklabels(['Off', 'On'])
        axis.set_xlabel('Switch')
        axis.set_ylabel('Reward')
