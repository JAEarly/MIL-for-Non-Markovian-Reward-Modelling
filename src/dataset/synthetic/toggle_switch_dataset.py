...
class ToggleSwitchOracleDataset(OracleDataset):
    ...
    def load_data(cls, csv_path=None):
        ...
        for bag_num in range(len(bag_dict)):
            ...
            switch_state = bag[:, 0]
            just_changed_0_to_1 = torch.zeros((len(bag)), dtype=torch.long)
            just_changed_1_to_0 = torch.zeros((len(bag)), dtype=torch.long)
